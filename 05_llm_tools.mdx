...
在上一章[GitHub文件爬虫](04_github_crawler.mdx)中，我们学习了如何自动从GitHub仓库获取文件。现在我们有了代码，我们需要一种方法来*理解*它。这就是大语言模型(LLMs)的用武之地！但直接与LLM交谈可能很复杂。这就是为什么我们有大语言模型交互工具。
...
除了简单的直接调用外，我们还可以使用`call_llm`函数处理更复杂的场景：

1. **提供文件内容分析**：向LLM提供代码文件，请求识别关键抽象和概念

2. **生成教程章节**：为识别出的每个抽象创建结构化教程章节

3. **分析抽象之间的关系**：确定不同组件如何相互作用，以便教程可以以逻辑顺序呈现概念

**在我们项目中的应用**

在我们的教程生成项目中，大语言模型交互工具是多个节点的核心组件：

* **IdentifyAbstractions节点**使用它来分析代码并识别关键抽象
* **AnalyzeRelationships节点**使用它来确定这些抽象之间的关系
* **WriteChapters节点**使用它为每个抽象生成教程章节

LLM实际上是整个教程生成过程的"大脑"，而`call_llm`函数使我们能够以结构化和可控的方式访问这个大脑。

**总结**

大语言模型交互工具是我们教程生成项目的核心组件。它使我们能够与大语言模型交互，以分析代码和生成教程内容。通过提供缓存、日志记录和认证等功能，它简化了与LLM交互的过程，让我们可以专注于提出正确的问题，而不是处理技术细节。

随着LLMs的能力不断发展，我们的交互工具将允许我们利用这些改进，而无需对我们的代码做出重大更改。我们可以简单地更新`call_llm`函数以使用新的模型或API，而其余的代码保持不变。

现在我们了解了如何与LLM交互以分析代码和生成内容，让我们探索我们项目的最后一个关键部分：[节点抽象](06_node_abstraction.mdx)，它将所有这些组件捆绑在一起以创建我们的教程生成流程。
